...
public:
    explicit Impl(const std::filesystem::path& path) {
        ...
        parse_added();
        compile_regex();
        if (vocab_.size() != 148899 || merge_ranks_.size() != 148643 ||
            added_.size() != 701 || reverse_.size() != 151552) {
            throw std::runtime_error("official tokenizer identity mismatch");
        }
    }
...
private:
    ...
    void parse_vocab() {
        ...
        cursor.expect('{');
        reverse_.resize(151552);
        is_added_.assign(151552, false);
        if (!cursor.consume('}')) {
            for (;;) {
                std::string token = cursor.string();
                cursor.expect(':');
                const std::uint32_t id = cursor.uint32();
                if (id >= reverse_.size()) throw std::runtime_error("base token id outside vocabulary");
    ...
    void parse_merges() {
        ...
        if (!cursor.consume(']')) {
            for (;;) {
                cursor.expect('[');
                const std::string left = cursor.string();
                cursor.expect(',');
                const std::string right = cursor.string();
                cursor.expect(']');
    ...
    void parse_added() {
        ...
        if (!cursor.consume(']')) {
            for (;;) {
                ...
                if (content.empty() || id >= reverse_.size()) throw std::runtime_error("invalid added token");
                reverse_[id] = content;
                is_added_[id] = true;
    ...
    void compile_regex() {
        static constexpr char pattern[] =
            R"P92('(?i:[sdmt]|ll|ve|re)| (?=\p{Han}|[＂＃＄％＆＇（）＊＋，－／：；＜＝＞＠［＼］＾＿｀｛｜｝～｟｠｢｣､　、〃〈〉《》「」『』【】〔〕〖〗〘〙〚〛〜〝〞〟〰〾〿–—‘’‛“”„‟…‧﹏﹑﹔·．！？｡。])|[＂＃＄％＆＇（）＊＋，－／：；＜＝＞＠［＼］＾＿｀｛｜｝～｟｠｢｣､　、〃〈〉《》「」『』【】〔〕〖〗〘〙〚〛〜〝〞〟〰〾〿–—‘’‛“”„‟…‧﹏﹑﹔·．！？｡。]+[\r\n]*|[^\r\n\p{L}\p{N}]?+[\p{L}\p{M}]+|\p{N}| ?[^\s\p{L}\p{N}]++[\r\n]*|\s*[\r\n]|\s+(?!\S)|\s+)P92";
        int error = 0;
        PCRE2_SIZE offset = 0;
        regex_ = pcre2_compile(reinterpret_cast<PCRE2_SPTR>(pattern), PCRE2_ZERO_TERMINATED,
                               PCRE2_UTF | PCRE2_UCP, &error, &offset, nullptr);
        if (regex_ == nullptr) throw std::runtime_error("cannot compile official tokenizer regex");
    }
...
std::string Tokenizer::chat_prompt(std::string_view user,
